    ...
    public function generate(\SplFileObject $file)
    {
        ...
        if($this->isImage($this->out_format)){
            ...
            elseif($this->isPDF($file->getExtension())){
                $preview_path = $this->generatePreview($orig_path.'['.$this->page_range.']', $preview_path, self::PDF_RESOLUTION);
            }
            // Other format to image
            else{
                $pdf_path = $this->convert($orig_path, self::PREVIEW_FORMAT_PDF);
				$preview_path = $this->generatePreview($pdf_path.'['.$this->page_range.']', $preview_path, self::PDF_RESOLUTION);
                // Remove temp files
